...
#pragma mark - Toolkit Function Implementation
    ...
    // to a copy of the removed item.
    {
        ...
        {	
            ...
            oldroot_ptr = root_ptr; // assign old root as root to be deleted
            removed = oldroot_ptr->data(); // assign data to removed
            if(root_ptr->left() != NULL){
            	root_ptr = root_ptr ->left(); // re init pointers to ensure tree is maintained
            }else{
            	root_ptr = NULL; // init to nothing then
            }
            delete oldroot_ptr; // delete the old root
        }
    }
    ...
    // function returns false).
    {
        ...
        {   // Target was found and there is no left subtree, so we can
        	...
        	oldroot_ptr = root_ptr; // assign old root as root to be deleted
        	root_ptr = root_ptr ->right(); // re init pointers to ensure tree is maintained
        	delete oldroot_ptr; // delete the old root
        	return true;
        }
...
#pragma mark - Member Function Implementation
    ...
    {
        root_ptr = tree_copy(source.root_ptr); // use tree copy to copy all elts from one to another
    }
    ...
    // Header file used: bintree.h
    {
        ...
        do
        {
            ...
            {   // Go left
            	if(cursor->left() != NULL){
            		cursor = cursor->left(); // go down left tree
            	}else{
            		done = true; // no more node to traverse
            ...
            {   // Go right
            	if(cursor->right() != NULL){
            		cursor = cursor->right(); // go down right tree
            	}else{
            		done = true; // no more node to traverse
    ...
    {
        if(this == &source){
        	return; // if trying to assign to itself dont do anything
        }

        tree_clear(root_ptr); // empty the tree
        root_ptr = tree_copy(source.root_ptr); // copy all elements into root
    ...
    void bag<Item>::operator +=(const bag<Item>& addend)
    {
        ...
        {
        	bag<Item> copiedBag = addend; // taking care of self assignment
            insert_all(copiedBag.root_ptr); // copy all elements into the new tree
        }
        else{
            insert_all(addend.root_ptr); // copy all the elements into new tree
        }
    }
    ...
    bag<Item> operator +(const bag<Item>& b1, const bag<Item>& b2)
    {
    	bag<Item> newBag = b1; // assign b1 to newBag
    	newBag += b2; // add b2 to newBag
        return newBag; // return the answer
    }
    ...
    // method.
    {
        if (addroot_ptr != NULL)
        {
            insert(addroot_ptr->data()); // add a first node as a root
            insert_all(addroot_ptr->left()); // insert all other left subtree nodes
        	insert_all(addroot_ptr->right()); // insert all other right subtree nodes
        }
    }
