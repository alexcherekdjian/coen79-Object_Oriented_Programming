...
namespace coen79_lab9
{
    ...
    // Library facilities used: cstdlib
    {
        if (node_ptr != NULL)
        {
        	inorder(f, node_ptr->left()); // recurse on left
        	f(node_ptr->data()); // print data
        	inorder(f, node_ptr->right()); // recurse on right
        }
    }
    ...
    // Library facilities used: cstdlib
    {
    	postorder(f, node_ptr->left()); // recurse on left
    	postorder(f, node_ptr->right()); // recurse on right
    	f(node_ptr->data()); // print data
    ...
    {	
        if (node_ptr != NULL){
	    	f(node_ptr->data()); // print data
	    	preorder(f, node_ptr->left()); // recurse on left
	    	preorder(f, node_ptr->right()); // recurse on right
    	}	
    }
    ...
    // Library facilities used: cstdlib
    {
        ...
        if (root_ptr != NULL)
        {
            ...
            child = root_ptr->right(); // re init child to point to right child
            tree_clear(child); // clear the subtree
            delete root_ptr; // delete the pointer
            root_ptr = NULL; // assign null
        }
    }
    ...
    // Library facilities used: cstdlib
    {
        ...
        {
			l_ptr = tree_copy(root_ptr->left()); // recurse on left
			r_ptr = tree_copy(root_ptr->right()); // recurse of right
			binary_tree_node<Item>* newNode = new binary_tree_node<Item>(root_ptr->data(), l_ptr, r_ptr); // create a new node
        	return newNode; // return and insert that node
        }
    }
    ...
    // Library facilities used: cstdlib
    {
        ...
        if (node_ptr == NULL)
            ...
            answer = 1 + tree_size(node_ptr->left()) + tree_size(node_ptr->right());
        	return answer;
    }
}
