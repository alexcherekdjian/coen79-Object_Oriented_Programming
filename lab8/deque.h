...
//   deque(int init_bp_array_size = BLOCKPOINTER_ARRAY_SIZE,
//         int initi_block_size = BLOCK_SIZE);
...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        block_pointers = new value_type* [bp_array_size]; // create new block pointer
        block_pointers_end = block_pointers + (bp_array_size - 1); // set back pointer
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                {
                    *(block_pointers[bp_array_index]+block_item_index) = *(source.block_pointers[bp_array_index]+block_item_index); // assign value to index

                    if(source.front_ptr == (source.block_pointers[bp_array_index] + block_item_index)){
                        front_ptr = block_pointers[bp_array_index] + block_item_index; // if this is source front_ptr assign to new deque
                    }

                    if(source.back_ptr == (source.block_pointers[bp_array_index] + block_item_index)){
                        back_ptr = block_pointers[bp_array_index] + block_item_index; // if this is source back_ptr assign to new deque
                    }
                }
            }
        }
    ...
    deque<Item>::~deque () {
        ...
        first_bp = last_bp = block_pointers_end = block_pointers = NULL; // re init pointers
        front_ptr = back_ptr = NULL;
    }
    ...
    void deque<Item>::clear () {
        ...
        for (size_type i = 0; i < bp_array_size; ++i){
            delete[] block_pointers[i]; // delete
            block_pointers[i] = NULL; // init to NULL
        }
        
        first_bp = last_bp = NULL; // re init pointers
        front_ptr = back_ptr = NULL; // re init pointers
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            *first_bp = new value_type [block_size]; // create a new block
            *(*first_bp) = entry; // insert value into first slot
            front_ptr = back_ptr = (*first_bp); // init front and back to equal new element
        ...
        {
            --front_ptr; // decrement pointer to point to empty slot before
            *front_ptr = entry; // insert entry into slot
        ...
        {
            --first_bp; // have first_bp point to slot above
            *first_bp = new value_type [block_size]; // allocate new block
            front_ptr = (*first_bp) + block_size - 1; // re initialize front_ptr to end of block
            *front_ptr = entry; // insert new elt at front_ptr
        }
        ...
        {
            reserve(); // reserve more space (does all copying and pointer adjustments)
            --first_bp; // have first_bp point to block above
            *first_bp = new value_type [block_size]; // allocate new block
            front_ptr = (*first_bp) + block_size - 1; // re initialize front_ptr to end of block
            *front_ptr = entry; // insert new elt at front_ptr
        }
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        if (last_bp == NULL)
        {
            ...
            last_bp = first_bp = block_pointers + bp_mid  - 1; // re init the last block pointer

            *last_bp = new value_type [block_size]; // create a new block
            *(*last_bp) = entry; // insert value into front slot
            front_ptr = back_ptr = (*last_bp); // init front and back to equal new element
        ...
        {
            ++back_ptr; // increment pointer to point to empty slot after
            *back_ptr = entry; // insert entry into slot
        }
        ...
        {
            ++last_bp; // have last_bp point to slot below
            *last_bp = new value_type [block_size]; // allocate new block
            back_ptr = (*last_bp); // re initialize back_ptr to beggining of new block
            *back_ptr = entry; // insert new elt at front_ptr
        }
    ...
    void deque<Item>::pop_back()
    {
        ...
        if (back_ptr == front_ptr)
        {
            clear(); // if equal get rid of everything
        }
        else if (back_ptr == *last_bp)
        {
            delete[] *last_bp; // delete last block
            *last_bp = NULL; // re init to NULL
            --last_bp; // decrement last block pointer
            back_ptr = *(last_bp) + block_size -1; // re init the back ptr to new back
        }
        else
        {
            --back_ptr; // still items so just decrement to point to previous
        }
    }
