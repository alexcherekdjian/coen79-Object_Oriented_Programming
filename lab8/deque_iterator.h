...
namespace coen79_lab8
{
    ...
    public:
        ...
        {
            //std::cout<< front_ptr << " " <<cursor << std::endl;
            ...
            {
                ++current_block_pointer; // increment current block ptr
                cursor = *current_block_pointer; // assign cursor to first elt of that block
                current_boundary = (*current_block_pointer) + block_size - 1; // re init boundary

            }
            // Move forward
            else
            {
                ++cursor; // increment cursor to next value
            }
